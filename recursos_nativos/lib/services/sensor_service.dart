...
class SensorService {
  ...
  void startShakeDetection(Function() onShake) {
    if (!_isSupportedPlatform) {
      debugPrint('⚠️ Sensores não suportados nesta plataforma (${Platform.operatingSystem}).');
      return;
    }

    if (_isActive) {
      debugPrint('⚠️ Detecção de shake já ativa.');
      return;
    }
  ...
  Future<void> _vibrateDevice() async {
    try {
      ...
      debugPrint('⚠️ Vibração não suportada: $e');
    }
  }
